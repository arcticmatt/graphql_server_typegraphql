...
let schema;
beforeAll(async () => {
    schema = await starWarsSchema_1.default();
});
describe("Star Wars Query Tests", () => {
    describe("Custom", () => {
      ...
      `;
            const result = await graphql_1.graphql({ schema, source });
    ...
    describe("Using IDs and query parameters to refetch objects", () => {
      ...
      `;
            ...
            const result = await graphql_1.graphql({ schema, source, variableValues });
            expect(result).toEqual({
                data: {
                    human: {
                        name: "Luke Skywalker",
      ...
      `;
            ...
            const result = await graphql_1.graphql({ schema, source, variableValues });
            // Behavior for this differs from reference.
            expect(result).toEqual({
                data: {
                    human: {
                        name: null,
